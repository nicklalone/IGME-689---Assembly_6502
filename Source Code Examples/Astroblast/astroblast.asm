...
;
; NOTE: You must compile this with vcsh version 105 or greater
;
TIA_BASE_READ_ADDRESS = $00         ; set the read address base so this runs on
   ...
   include "vcsh"
   include "macroh"
   include "tia_constantsh"

;
; Make sure we are using vcsh version 105 or greater
;
   IF VERSION_VCS < 105
   
      echo ""
      echo "*** ERROR: vcsh file *must* be version 105 or higher!"
      echo "*** Updates to this file, DASM, and associated tools are"
      echo "*** available at https://dasm-assemblergithubio/"
      echo ""
      err
      
   ENDIF
;
; Make sure we are using macroh version 101 or greater
;
   IF VERSION_MACRO < 101

      echo ""
      echo "*** ERROR: macroh file *must* be version 101 or higher!"
      echo "*** Updates to this file, DASM, and associated tools are"
      echo "*** available at https://dasm-assemblergithubio/"
      echo ""
      err
...
DetermineObjectHorizOffset
   ...
   rts
;
; Horizontal reset starts at cycle 23 (ie pixel 69) for objects on the left
; side of the screen and cycle 48 (ie pixel 144) for objects on the right side
...
launchMissile
   ldaw laserBaseHorizValue        ; get laser base horizontal value
   sta playerHorizValues,x          ; set it to missile horizontal value
   ldaw laserBaseHorizPosition     ; get laser base horizontal position
...
continueDetermineControllerType
   lda #DISABLE_TIA
   sta VBLANK                       ; disable TIA (ie D1 = 1)
   ldx #32                          ; wait ~538 scanlines (ie ~2 frames)
outerLoopWait
   ldy #0
innerLoopWait
   iny
